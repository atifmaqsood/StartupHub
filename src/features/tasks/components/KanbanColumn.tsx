...
const KanbanColumn: React.FC<KanbanColumnProps> = ({ id, title, tasks, color }) => {
  ...
  return (
    <div className="flex flex-col w-80 min-w-[320px] max-w-sm h-full bg-gray-50/50 rounded-2xl p-4 border border-transparent transition-colors">
      ...
      <div
        ref={setNodeRef}
        className={cn(
          "flex-1 space-y-4 overflow-y-auto min-h-[200px] rounded-xl transition-all duration-200",
      ...
      >
        ...
        {tasks.length === 0 && !isOver && (
          <div className="text-gray-300 text-sm font-medium border-2 border-dashed border-gray-100 rounded-xl p-8 text-center w-full">
            No tasks here
          </div>
        )}
      </div>
